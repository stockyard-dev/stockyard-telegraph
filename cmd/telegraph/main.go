// Stockyard Telegraph — Webhook sender.
// Define events, register subscriber URLs, fire and deliver with signing and retry.
...
import (
	...
	"github.com/stockyard-dev/stockyard-telegraph/internal/license"
...
func main() {
	...
	licenseKey := os.Getenv("TELEGRAPH_LICENSE_KEY")
	licInfo, licErr := license.Validate(licenseKey, "telegraph")
	...
	if licInfo != nil && licInfo.IsPro() {
		...
		log.Printf("  License:   Free tier (set TELEGRAPH_LICENSE_KEY to unlock Pro)")
	}
	...
	log.Printf("")
	log.Printf("  Stockyard Telegraph %s", version)
	log.Printf("  Fire:           POST http://localhost:%d/api/events/{name}/fire", port)
	log.Printf("  API:            http://localhost:%d/api/events", port)
	...
	go func() {
		for {
			...
			if err != nil {
				...
				log.Printf("[cleanup] deleted %d deliveries older than %d days", n, retentionDays)
			}
		}
	}()
